...
def plot_training_curves(train_metrics, val_metrics, val_obst_metrics, save_path):
    ...
    if "recon_l1" in train_metrics and "residual_l1" in train_metrics:
        ...
        ax2_twin = ax2.twinx()
        _plot_metric_series(ax2_twin, train_x, train_metrics.get("pca_coord_mse", []), label="Train PCA Coord MSE", color="cyan", linewidth=1.8)
        _plot_metric_series(ax2_twin, val_x, val_metrics.get("pca_coord_mse", []), label="Val (Normal) PCA Coord MSE", color="magenta", linewidth=2.0, marker="o")
        _plot_metric_series(ax2_twin, val_obst_x, val_obst_metrics.get("pca_coord_mse", []), label="Val (Obstacle) PCA Coord MSE", color="gray", linewidth=2.0, marker="o")
